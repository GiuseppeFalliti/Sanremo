...
<div class="relative absolute insert-0 top-20">
    ...
    <script>
        ...
        // Funzione mock per simulare la chiamata API (sostituire con chiamata reale al backend)
        async function fetchSearchResults(query) {
            // Simula un ritardo di rete
            await new Promise(resolve => setTimeout(resolve, 300));

            // Mock data - sostituire con dati reali dal tuo backend
            return [{
                    type: 'song',
                    title: 'La noia',
   ...
   // funzione per creare gli elementi dei risultati  della barra di ricerca 
        function createResultItem(result) {
            ...
            if (result.type === 'song') {
                ...
                <div class="flex items-center space-x-4">
                    <div class="flex-1">
                        ...
                        <p class="text-black/80 text-sm">${result.artist}</p>
                    </div>
                    <span class="text-black/60 text-sm">${result.listens}</span>
                </div>
            `;
            }
            // Se il risultato è un artista 
            else {
                item.innerHTML = `
                <div class="flex items-center space-x-4">
                    <div class="flex-1">
                        <p class="text-black font-semibold">${result.name}</p>
                        <p class="text-black/80 text-sm">Canzoni: ${result.songs.join(', ')}</p>
                    </div>
            ...
            item.addEventListener('click', () => {
                // Aggiungi qui la logica per reindirizzare alla pagina specifica
        ...
        const handleSearch = debounce(async (e) => {
            ...
            try {
                ...
                if (results.length === 0) {
                    resultsContainer.innerHTML = '<p class="text-white/80 text-center">Nessun risultato trovato</p>';
                } else {
